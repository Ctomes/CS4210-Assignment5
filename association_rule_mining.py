...
# SPECIFICATION: A program that uses apriori module to produce association rules from the data: retail_dataset.csv
...
for index, row in df.iterrows():
    ...
    labels = {item: False for item in itemset}
...
transactions = len(encoded_vals)
dfev = pd.DataFrame(encoded_vals)
print(dfev)
for index, rule in rules.iterrows():
    ...
    print(str(antecedents) + " -> " + str(consequents))
...
#-->add your python code below
    ...
    support_count = (dfev[consequents] == False).all(axis=1).sum()
    prior = support_count/transactions
    gain = (100*(rule.confidence-prior)/prior)
